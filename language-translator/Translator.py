...
def english_sentences() -> list[str]:
    ...
    """
    # TODO: load
    return load_data('data/small_vocab_en')
...
def french_sentences() -> list[str]:
    ...
    """
    # TODO: load
    return load_data('data/small_vocab_fr')
...
def preprocess(x: List[str], y: List[str]):
    ...
    for i, v in enumerate(preprocess_x):
        ...
        if v not in arr_x:
            continue
        for j in range(len(preprocess_x)):
            if j == i:
                continue
            try:
...
def final_predictions(x, y, x_tk, y_tk, english_sentences, french_sentences) -> None:
    ...
    history_model = model.fit(X_train, Y_train, batch_size=800, epochs=2, callbacks=[es],
